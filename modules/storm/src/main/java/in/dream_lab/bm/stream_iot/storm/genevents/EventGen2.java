...
public class EventGen2 {
	...
	public void launch(String csvFileName, String outCSVFileName, long experimentDurationMillis) {
		...
		try {
			 ...
			 * " splitted csv files to memory. nestedList"); for (TableClass c :
			 * nestedList) System.out.println(this.getClass().getName() + " " +
			 * c.toString() + " \n + Header" + c.getHeader() + "\n # Rows" +
			 * c.getRows());
			 */
			...
			for (int i = 0; i < numThreads; i++) {
				...
				// System.out.println(this.getClass().getName() + " submitting
				// first runnable");
	...
	public TableClass launch(String csvFileName, String outCSVFileName, long experimentDurationMillis,
		...
		try {
			 ...
			 * Semaphore sem2 = new Semaphore(0);
			 * 
			 * SubEventGen[] subEventGenArr = new SubEventGen[numThreads]; for
			 * (int i = 0; i < numThreads; i++) { //
			 * this.executorService.execute(new SubEventGen(this.iseg, //
			 * nestedList.get(i))); subEventGenArr[i] = new
			 * SubEventGen(this.iseg, nestedList.get(i), sem1, sem2, this.rate);
			 * this.executorService.execute(subEventGenArr[i]); }
			 * 
			 * sem1.acquire(numThreads); // set the start time to all the thread
			 * objects long experiStartTs = System.currentTimeMillis(); for (int
			 * i = 0; i < numThreads; i++) { // this.executorService.execute(new
			 * SubEventGen(this.iseg, // nestedList.get(i)));
			 * subEventGenArr[i].experiStartTime = experiStartTs; if
			 * (experimentDurationMillis > 0) subEventGenArr[i].experiDuration =
			 * experimentDurationMillis;
			 * this.executorService.execute(subEventGenArr[i]); }
			 * sem2.release(numThreads);
			 */ } catch (IOException e) {
			// TODO Auto-generated catch block
...
class SubEventGen2 implements Runnable {
	ISyntheticEventGen iseg;
	...
	public SubEventGen2(ISyntheticEventGen iseg, TableClass eventList, Semaphore sem1, Semaphore sem2) {
		this.iseg = iseg;
	...
	public SubEventGen2(ISyntheticEventGen iseg, TableClass eventList, Semaphore sem1, Semaphore sem2, int rate) {
