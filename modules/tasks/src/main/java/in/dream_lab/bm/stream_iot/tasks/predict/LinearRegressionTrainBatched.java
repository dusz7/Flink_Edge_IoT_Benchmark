...
/**
 * This task should only be run from a single thread to avoid overwriting output
 * model file.
 * 
...
public class LinearRegressionTrainBatched extends AbstractTask {
	...
	public void setup(Logger l_, Properties p_) {
		// TODO: Later, have option of training using instances present in data
		// file rather than just from messages
		super.setup(l_, p_);
	...
	protected Float doTaskLogic(Map map) {
		...
		String fullFilePath = modelFilePath + filename; // model file updated
														// with
														// MLR-endRowkey.model
		int result = 0;
		try {
...
/*			if (l.isInfoEnabled())
	 ...
	 * @return
	 */
	private static int linearRegressionTrainAndSaveModel(StringReader instanceReader, String modelFilePath,
			ByteArrayOutputStream model, Logger l) {
