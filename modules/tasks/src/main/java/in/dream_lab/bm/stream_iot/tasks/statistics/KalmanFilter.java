	...
	public void setup(Logger l_, Properties p_) {
		...
		synchronized (SETUP_LOCK) { // ONLY for static fields
			if (!doneSetup) { // Do setup only once for this task
				...
				// If positive, use that particular field number in the input
				// CSV message as input for count else random value
		...
		// Init Non-Static fields //30. Why?
		p0_priorErrorCovariance = Float.parseFloat(p_.getProperty("STATISTICS.KALMAN_FILTER.ESTIMATED_ERROR", "1"));
	}

	/**
	 * Based on applying Kalman Filter for a voltage sensor at: Kalman Filter
	 * For Dummies, Bilgin Esme
